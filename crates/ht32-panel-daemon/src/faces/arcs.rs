...
///
/// Produced by [`ArcsFace::arc_gauge_specs`] and [`ArcsFace::activity_arc_specs`] and
/// consumed by `layout()` (as `WidgetContent::Arc`), centralizing the floating-point
/// computation so the gauge geometry is defined in exactly one place.
...
mod tests {
    ...
    fn arcs_layout_matches_render_landscape_with_ip() {
        let via_layout = render_both_with(320, 170, sample_with_ip("192.168.1.100"));
        assert_eq!(
            pixel_hash(&via_layout),
    ...
    fn arcs_layout_matches_render_landscape_ipv6() {
        let via_layout = render_both_with(320, 170, sample_with_ip("2001:db8::dead:beef:1:2"));
        assert_eq!(
            pixel_hash(&via_layout),
            2326329289435487373,
    ...
    fn arcs_layout_matches_render_portrait_with_ip() {
        let via_layout = render_both_with(170, 320, sample_with_ip("192.168.1.100"));
        assert_eq!(
            pixel_hash(&via_layout),
    ...
    fn arcs_layout_matches_render_portrait_ipv6() {
        let via_layout = render_both_with(170, 320, sample_with_ip("2001:db8::dead:beef:1:2"));
        assert_eq!(
            pixel_hash(&via_layout),
    ...
    fn arcs_layout_matches_render_landscape_analogue() {
        let via_layout = render_both_comps(320, 170, sample(), analogue_comps());
        assert_eq!(
            pixel_hash(&via_layout),
            4911362616083430275,
    ...
    fn arcs_layout_matches_render_portrait_analogue() {
        let via_layout = render_both_comps(170, 320, sample(), analogue_comps());
        assert_eq!(
            pixel_hash(&via_layout),
            8763038846165929711,
